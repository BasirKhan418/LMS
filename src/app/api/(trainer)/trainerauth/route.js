...
export const POST= async(req,res)=>{
  ...
  if(reqdata.type=="send"){
   try{
    ...
    let data = await Trainer.find({email:reqdata.email});
    ...
    const info = await transporter.sendMail({
      from: '"Infotact Lms Trainer Login" <account@infotactlearning.in>',
      to: `${reqdata.email}`, // list of receivers
      subject: `Your ${process.env.BRAND} Lms Trainer Login OTP: Secure Access Code Inside`, // Subject line
      ...
      <body style="font-family: Arial, sans-serif; background-color: #f4f4f4; margin: 0; padding: 0;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); overflow: hidden;">
              <div style="background-color: #007bff; color: #ffffff; padding: 20px; text-align: center;">
                  <h1 style="margin: 0;">Trainer Login Verification</h1>
              </div>
  ...
  //if type is verify then verify otp
   ...
   try{
    ...
    if(otpdata.otp==reqdata.otp){
      ...
      let tokemstore = await Trainer.findOneAndUpdate({email:reqdata.email},{token:token})
