...
const Page = () => {
  ...
  const validateUser = async () => {
    try {
      ...
      if (res.success) {
        ...
        setUser(res.user || null)  // Ensure null if undefined
        setData(res.data || [])    // Ensure empty array if undefined
        setBatchdetails(res.batch || null)  // Ensure null if undefined
      ...
      // If error occurs during authentication, redirect to login
      router.push("/login")
    }
  }
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (response.success) {
        setCourses(response.data || [])  // Ensure empty array if data is undefined
  ...
  useEffect(() => {
    // Chain these calls to ensure user validation happens first
